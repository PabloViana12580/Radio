...
public class GUI{
    ...
    public GUI(){
        ...
        if (grupo.getEstado()) {
            ...
            //panel que muestra emisora y frecuencia
            JPanel panelEmisora = new JPanel();
            panelEmisora.setLayout(new FlowLayout());
            JLabel emisora = new JLabel(decimales.format(grupo.getEmisora()) + " " + verificador(grupo.getFrecuencia()));
            ...
            ActionListener listener = new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                    ...
                    emisora.setText(decimales.format(grupo.getEmisora()) + " " + verificador(grupo.getFrecuencia()));
                }
            };
            ...
            ActionListener cambio = new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                    if (((JButton) e.getSource()).getName() == ">>") {
                        grupo.frecuencia(); //cambia frecuencia segun boton apachado
                    } else {
                        grupo.frecuencia();
                    }
                    emisora.setText(decimales.format(grupo.getEmisora()) + " " + verificador(grupo.getFrecuencia()));
            ...
            */
            ...
            //actualizar un Label y cambiar Frecuencia
            ActionListener cambioFrecuencia = new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                    if (grupo.getFrecuencia()) {
                        grupo.setEmisora(530);
                        grupo.cambiar(true); //cambia frecuencia FM a AM y viceversa
                    } else {
                        grupo.setEmisora((float)87.9);
                        grupo.cambiar(false);
                    }
            ...
            //panel de botones de cambio de frecuencia
            JPanel panelFrecuencia = new JPanel();
            panelFrecuencia.setLayout(new FlowLayout());
            JButton AMFM = new JButton("FM/AM");
            AMFM.setName("FM/AM");
            AMFM.addActionListener(cambioFrecuencia);
            panelFrecuencia.add(AMFM);
            ...
            f.add(panelFrecuencia, BorderLayout.CENTER);
    ...
    //devuelve la frecuencia utilizada
    static String verificador(boolean frecuencia) {
        if (frecuencia) {
            return "FM";
        } else {
            return "AM";
